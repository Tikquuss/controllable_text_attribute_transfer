...
class EncoderDecoderAE(EncoderDecoderBase):
    """
    A standard Encoder-Decoder architecture. Base for this and many other models.
    """
    def __init__(self, encoder, decoder, src_embed, tgt_embed, generator, position_layer, model_size, 
                latent_size):
        super(EncoderDecoderAE, self).__init__(encoder, decoder, src_embed, tgt_embed, generator, position_layer, model_size, 
                latent_size)
...
class EncoderDecoderAEGRU(EncoderDecoderBase):
    """
    A standard Encoder-Decoder architecture. Base for this and many other models.
    """
    def __init__(self, encoder, decoder, src_embed, tgt_embed, generator, position_layer, model_size, 
                latent_size):
        super(EncoderDecoderAEGRU, self).__init__(encoder, decoder, src_embed, tgt_embed, generator, position_layer, model_size, 
                latent_size)
        d_model = model_size
    ...
    def forward(self, src, tgt, src_mask, src_attn_mask, tgt_mask, return_intermediate=False):
        ...
        """
        if return_intermediate :
            memory, z = self.encode(src, src_mask, return_intermediate)  # (batch_size, max_src_seq, d_model)
        else :
...
class EncoderDecoderAR(EncoderDecoderBase):
    """
    A standard Encoder-Decoder architecture. Base for this and many other models.
    """
    def __init__(self, encoder, decoder, src_embed, tgt_embed, generator, position_layer, model_size, 
                latent_size):
        super(EncoderDecoderAR, self).__init__(encoder, decoder, src_embed, tgt_embed, generator, position_layer, model_size, 
                latent_size)
        self.sigmoid = nn.Sigmoid()
        self.decoder = None
        self.tgt_embed = None
...
def make_model(d_vocab, N, d_model, latent_size, d_ff=1024, h=4, dropout=0.1):
    ...
    #training_task = "ar"
    if training_task == "ae" :
        model_class = EncoderDecoderAE
        #model_class = EncoderDecoderAEGRU
    if training_task == "ar" :
        model_class = EncoderDecoderAR
